...
function resetFromClient() {
    name.value = props.client.name;
    description.value = props.client.description ?? '';
    contactRows.value = props.client.contacts.map((c) => ({ ...c }));
}

watch(
    () => [show.value, props.client],
    () => {
        if (show.value) {
            resetFromClient();
        }
...
async function submit() {
    ...
    await updateClient(props.client.id, body);
    show.value = false;
}
